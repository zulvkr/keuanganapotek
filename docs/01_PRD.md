...
Aplikasi ini dibangun untuk menyederhanakan, mengotomatisasi, dan mempercepat pencatatan akuntansi apotek menggunakan pendekatan **High-Speed Data Grid** dengan prinsip _double-entry accounting_ otomatis.
...
3. **Pencegahan Human Error (Error Prevention):** Validasi keseimbangan debit-kredit secara visual real-time (_sticky bottom bar_), deteksi duplikasi nomor faktur PBF, dan peringatan selisih kas fisik.
...
5. **Integritas Saldo Awal:** Fitur cut-off saldo awal dengan validasi keseimbangan dan opsi alokasi selisih otomatis ke ekuitas (_Auto-Balancing Equity_).
...
- Mode Saldo Awal (_Opening Balance_) dengan validasi Total Debit = Total Kredit per tanggal cut-off.
- Auto-generate Jurnal Pembuka (_Opening Balance Journal_) dan sistem penguncian periode (_Lock State_).
...
- Input nilai modal obat (HPP/COGS) harian dan 1-click _Generate Jurnal Penjualan & HPP_.
...
- Fitur _Paste from Excel_ (`Ctrl + V`) multi-baris.
...
- Tampilan dua panel berdampingan (_Dual Pane Split View_): Mutasi Internal Sistem vs Rekening Koran Bank.
- Tombol _Auto-Match_ berbasis tanggal (±1 hari) dan nominal identik.
...
- Fitur interaktif _Drill-down_: Mengklik nilai akun membuka slide-over drawer berisi daftar jurnal pembentuk.
