...
export function assertPeriodUnlocked(db: Executor, entryDate: string): void {
  const lock = db.select({ lockedThrough: periodLocks.lockedThrough })
    .from(periodLocks).where(gte(periodLocks.lockedThrough, entryDate))
    .orderBy(asc(periodLocks.lockedThrough)).limit(1).get();
...
export function lockPeriod(db: Db, input: { lockedThrough: string; actor: string; role: PeriodLockRole }) {
  return db.transaction((tx) => {
    const existing = tx.select().from(periodLocks).where(eq(periodLocks.lockedThrough, input.lockedThrough)).get();
    if (existing) return existing;
    const row = {
      id: randomUUID(), lockedThrough: input.lockedThrough, lockedBy: input.actor,
      lockedByRole: input.role, lockedAt: nowIsoInstant(),
    } as const;
    tx.insert(periodLocks).values(row).run();
    recordAudit(tx, { entityType: "PERIOD", entityId: row.id, action: "LOCK", actor: input.actor, after: row });
    return row;
  });
}
...
export function unlockPeriod(db: Db, lockedThrough: string, actor: string) {
  return db.transaction((tx) => {
    const existing = tx.select().from(periodLocks).where(eq(periodLocks.lockedThrough, lockedThrough)).get();
    ...
    recordAudit(tx, { entityType: "PERIOD", entityId: existing.id, action: "UNLOCK", actor, before: existing });
    return existing;
  });
}
