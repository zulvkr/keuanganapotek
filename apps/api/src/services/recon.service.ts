...
function parseBankStatementCsv(csv: string): BankStatementImportRow[] {
  const lines = csv
    ...
    .filter(Boolean);
  if (lines.length < 2)
    throw new Error("CSV rekening koran harus memiliki header dan minimal satu baris data");
  const first = lines[0]!;
  ...
  if (dateIndex < 0 || debitIndex < 0 || creditIndex < 0)
    throw new Error("Header CSV wajib memuat tanggal, debit, dan kredit");
  return lines.slice(1).map((line, index) => {
    ...
    const parsed = BankStatementImportRowSchema.safeParse({
      statementDate: normalizeDate(cells[dateIndex] || ""),
      description: descriptionIndex >= 0 ? cells[descriptionIndex] : undefined,
      debit,
      credit,
    });
    if (!parsed.success)
      throw new Error(
        `Baris CSV ${index + 2} tidak valid: ${parsed.error.issues.map((issue) => issue.message).join(", ")}`,
      );
    return parsed.data;
  });
}

function mapMatchRows(
  db: Executor,
  statementRows: Array<typeof bankStatements.$inferSelect>,
): ReconStatementRow[] {
  ...
  const matches = db
    ...
    .where(
      inArray(
        bankReconMatches.bankStatementId,
  ...
  return statementRows.map((row) => {
    ...
    return {
      ...
      journalLineId: match?.journalLineId ?? null,
      matchType: match?.matchType ?? null,
    };
  });
}

export function importBankStatements(
  db: Db,
  input: BankStatementImport,
): { imported: ReconStatementRow[] } {
  ...
  const account = db
    ...
    .from(accounts)
    .where(eq(accounts.id, parsed.data.bankAccountId))
    .get();
...
function getInternalRows(
  ...
  const rows = db
    ...
    .orderBy(asc(journals.entryDate), asc(journalLines.lineNumber))
    .all();
  return rows.map((row) => ({
    journalLineId: row.journalLineId,
    journalId: row.journalId,
    entryDate: row.entryDate,
    description: row.lineDescription || row.memo || "Mutasi jurnal",
    referenceNo: row.referenceNo,
    debit: row.debit,
    credit: row.credit,
    isMatched: Boolean(row.matchId),
    matchId: row.matchId,
...
function matchingAmount(
  ...
  internal: { debit: number; credit: number },
): boolean {
  return statement.credit > 0
    ? internal.debit === statement.credit
    : internal.credit === statement.debit;
}
...
function createMatch(
  ...
  if (
    statement.isMatched ||
    db
      .select({ id: bankReconMatches.id })
      .from(bankReconMatches)
      .where(
        or(
          eq(bankReconMatches.bankStatementId, bankStatementId),
          eq(bankReconMatches.journalLineId, journalLineId),
        ),
      )
      .get()
  )
    throw new Error("Salah satu transaksi sudah dicocokkan");
  if (internal.accountId !== statement.bankAccountId)
    throw new Error("Akun bank internal dan rekening koran harus sama");
  if (!matchingAmount(statement, internal)) throw new Error("Nominal debit-kredit tidak identik");
  const id = randomUUID();
  db.insert(bankReconMatches)
    .values({ id, bankStatementId, journalLineId, matchType, matchedAt: nowIsoInstant() })
    .run();
  db.update(bankStatements)
    .set({ isMatched: true })
    .where(eq(bankStatements.id, bankStatementId))
    .run();
...
export function autoMatch(db: Db, bankAccountId: string) {
  const statements = db
    ...
    .from(bankStatements)
    .where(
      and(eq(bankStatements.bankAccountId, bankAccountId), eq(bankStatements.isMatched, false)),
    )
    .all();
  ...
  db.transaction((tx) => {
    for (const statement of statements) {
      const candidate = internal
        .filter(
          (line) =>
            !used.has(line.journalLineId) &&
            matchingAmount(statement, line) &&
            dateDistance(statement.statementDate, line.entryDate) <= 1,
        )
        .sort(
          (left, right) =>
            dateDistance(statement.statementDate, left.entryDate) -
            dateDistance(statement.statementDate, right.entryDate),
        )[0];
...
export function manualMatch(db: Db, input: { bankStatementId: string; journalLineId: string }) {
  ...
  return db.transaction((tx) =>
    createMatch(tx, parsed.bankStatementId, parsed.journalLineId, "MANUAL"),
  );
}
...
export function removeMatch(db: Db, matchId: string) {
  return db.transaction((tx) => {
    ...
    tx.update(bankStatements)
      .set({ isMatched: false })
      .where(eq(bankStatements.id, match.bankStatementId))
      .run();
    return { removed: true };
  });
}
