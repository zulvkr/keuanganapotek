...
describe("Phase 4 bank reconciliation", () => {
  it("imports a CSV and auto-matches an identical bank movement within one calendar day", async () => {
    ...
    const transfer = await api.request("http://localhost/api/cash-bank/transfers", {
      method: "POST",
      headers: json,
      body: JSON.stringify({
    ...
    const imported = await api.request("http://localhost/api/bank-recon/import", {
      method: "POST",
      headers: json,
      body: JSON.stringify({
    ...
    const matched = await api.request("http://localhost/api/bank-recon/auto-match", {
      method: "POST",
      headers: json,
      body: JSON.stringify({ bankAccountId: "coa-1111" }),
    });
  ...
  it("allows manual matching when the dates are more than one day apart", async () => {
    const { api, client } = setup();
    await api.request("http://localhost/api/cash-bank/transfers", {
      method: "POST",
      headers: json,
      body: JSON.stringify({
        transactionTime: "2026-09-01T09:00",
        transactionType: "DEPOSIT",
        sourceAccountId: "coa-1101",
        targetAccountId: "coa-1111",
        netAmount: "50.000",
    ...
    await api.request("http://localhost/api/bank-recon/import", {
      method: "POST",
      headers: json,
      body: JSON.stringify({
        bankAccountId: "coa-1111",
    ...
    const auto = await api.request("http://localhost/api/bank-recon/auto-match", {
      method: "POST",
      headers: json,
      body: JSON.stringify({ bankAccountId: "coa-1111" }),
    });
    ...
    const beforeBody = (await before.json()) as {
      data: { statements: Array<{ id: string }>; internal: Array<{ journalLineId: string }> };
    };
    const manual = await api.request("http://localhost/api/bank-recon/matches", {
      method: "POST",
      headers: json,
      body: JSON.stringify({
        bankStatementId: beforeBody.data.statements[0]!.id,
        journalLineId: beforeBody.data.internal[0]!.journalLineId,
      }),
    });
    expect(manual.status).toBe(201);
    expect(client.db.select().from(bankReconMatches).get()?.matchType).toBe("MANUAL");
    expect(
      client.db.select().from(journals).where(eq(journals.sourceModule, "CASH_BANK")).all(),
    ).toHaveLength(1);
  });
});
