...
import { autoBalanceOpeningBalances, deleteAccount as removeAccount, invalidateAccountingQueries, lockOpeningBalances, saveAccount, saveOpeningBalances } from "../lib/mutations";
...
function CoAPage() {
  ...
  const balancesQuery = useQuery({ queryKey: queryKeys.openingBalances(loadedCutoffDate), queryFn: () => getOpeningBalances(loadedCutoffDate), enabled: Boolean(metaQuery.data) });
  const loading = accountsQuery.isLoading || metaQuery.isLoading || balancesQuery.isLoading;
  useEffect(() => { if (metaQuery.data?.cutoffDate && !isCutoffDateDirty) setCutoffDate(metaQuery.data.cutoffDate); }, [metaQuery.data, isCutoffDateDirty]);
  ...
  const deleteMutation = useMutation({ mutationFn: removeAccount, onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: queryKeys.accounts }); }, onError: () => setMessage("Akun tidak dapat dihapus. Hapus sub-akun atau referensi terkait terlebih dahulu.") });
    ...
    <div className="space-y-5">
      <section className="space-y-4">
        ...
        <div aria-label="Mode CoA" className="flex gap-6 border-b border-slate-200" role="tablist">
          <button aria-selected={mode === "structure"} className={`border-b-2 px-1 pb-3 text-sm font-semibold transition-colors ${mode === "structure" ? "border-slate-900 text-slate-900" : "border-transparent text-slate-500 hover:border-slate-300 hover:text-slate-700"}`} onClick={() => setMode("structure")} role="tab" type="button">Struktur Akun</button>
          <button aria-selected={mode === "opening"} className={`border-b-2 px-1 pb-3 text-sm font-semibold transition-colors ${mode === "opening" ? "border-brand text-brand" : "border-transparent text-slate-500 hover:border-slate-300 hover:text-slate-700"}`} onClick={() => setMode("opening")} role="tab" type="button">Input Saldo Awal</button>
        </div>
      </section>

      {mode === "opening" && isCutoffDateDirty && cutoffDate !== loadedCutoffDate && !isLocked && <p className="rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800" role="status">Tanggal saldo awal diubah. Draft yang sedang diinput akan disimpan dengan tanggal {cutoffDate}.</p>}
      ...
      <section className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm md:flex-row md:items-center">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {mode === "opening" && <label className="flex items-center gap-2 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-900" htmlFor="cutoff"><span>Tanggal saldo awal</span><input aria-label="Tanggal saldo awal" className="rounded-md border border-blue-200 bg-white px-2 py-1 text-sm font-normal tabular-nums text-slate-700" disabled={isLocked} id="cutoff" onChange={(event) => { setCutoffDate(event.target.value); setIsCutoffDateDirty(true); }} type="date" value={cutoffDate} /></label>}
        </div>
      </section>
