...
function CoAPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`${apiBase}/api/opening-balances?cutoffDate=${cutoffDate}`).then((response) => response.json() as Promise<ApiResponse<BalanceResponseRow[]>>),
    ]).then(([accountResponse, balanceResponse]) => {
      if (cancelled) return;
