...
type ReportTotal = { amount: number; compareAmount: number; variance: number; growthPercent: number | null };
type IncomeSection = { key: string; label: string; rows: IncomeRow[]; total: ReportTotal };
type IncomeReport = { period: Period; comparePeriod: Period | null; sections: IncomeSection[]; revenue: ReportTotal; cogs: ReportTotal; grossProfit: ReportTotal; operatingExpenses: ReportTotal; nonOperatingExpenses: ReportTotal; netProfit: ReportTotal };
type BalanceRow = { accountId: string; code: string; name: string; balance: number };
type BalanceReport = { asOfDate: string; sections: Array<{ key: string; label: string; rows: BalanceRow[]; total: number }>; currentEarnings: number; totalAssets: number; totalLiabilities: number; totalEquity: number; difference: number; isBalanced: boolean };
type TrialRow = { accountId: string; code: string; name: string; classification: string; debitBalance: number; creditBalance: number; endingBalance: number };
type TrialReport = { period: Period; rows: TrialRow[]; totalDebit: number; totalCredit: number; difference: number; isBalanced: boolean };
type DrillDown = { account: { code: string; name: string }; entries: Array<{ lineId: string; journalNo: string; entryDate: string; referenceNo: string | null; memo: string | null; sourceModule: string; description: string | null; debit: number; credit: number }>; totalDebit: number; totalCredit: number };

const inputClass = "rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-mono tabular-nums outline-none focus:border-brand focus:ring-2 focus:ring-blue-100";
...
export default function ReportsPage() {
  ...
  function incomeView() {
    ...
    const summary = [{ label: "Pendapatan", value: income.revenue.amount }, { label: "Laba kotor", value: income.grossProfit.amount }, { label: "Laba bersih usaha", value: income.netProfit.amount }];
    return <><div className="grid gap-3 md:grid-cols-3">{summary.map((item) => <div className="rounded-xl border border-slate-200 bg-slate-50 p-4" key={item.label}><p className="text-xs text-muted">{item.label}</p><p className="mt-2 font-mono text-xl font-bold tabular-nums">Rp {formatMoney(item.value)}</p></div>)}</div><div className="space-y-4">{income.sections.map((section) => <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm" key={section.key}><div className="border-b border-slate-100 px-5 py-4"><h3 className="font-semibold">{section.label}</h3><p className="mt-1 text-xs text-muted">Klik nominal bulan berjalan untuk melihat jurnal pembentuk.</p></div><ComparativeTable openDrillDown={(row) => void openDrillDown(row.accountId)} section={section} /></section>)}</div></>;
  }

  function balanceView() {
    if (!balance) return null;
    return <><div className={"rounded-xl border p-4 " + (balance.isBalanced ? "border-emerald-200 bg-emerald-50" : "border-red-200 bg-red-50")}><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-semibold">Persamaan dasar akuntansi</p><p className="mt-1 text-xs text-muted">Aset = Kewajiban + Ekuitas termasuk laba periode berjalan.</p></div><span className={"font-mono text-sm font-bold tabular-nums " + (balance.isBalanced ? "text-emerald-700" : "text-red-700")}>{balance.isBalanced ? "✓ Seimbang" : "Selisih Rp " + formatMoney(balance.difference)}</span></div></div><div className="grid gap-4 md:grid-cols-3">{balance.sections.map((section) => <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm" key={section.key}><div className="border-b border-slate-100 px-5 py-4"><h3 className="font-semibold">{section.label}</h3><p className="mt-1 font-mono text-lg font-bold tabular-nums">Rp {formatMoney(section.total)}</p></div><table className="w-full text-sm"><tbody>{section.rows.map((row) => <tr className="border-b border-slate-50" key={row.accountId}><td className="px-5 py-3"><span className="font-mono text-xs tabular-nums text-slate-500">{row.code}</span><span className="ml-2">{row.name}</span></td><td className="px-5 py-3 text-right"><ReportAmount onClick={() => void openDrillDown(row.accountId, { startDate: "0001-01-01", endDate: balance.asOfDate })} value={row.balance} /></td></tr>)}</tbody></table></section>)}</div><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex flex-wrap gap-x-8 gap-y-3 text-sm"><span>Total aset <strong className="ml-2 font-mono tabular-nums">Rp {formatMoney(balance.totalAssets)}</strong></span><span>Total kewajiban <strong className="ml-2 font-mono tabular-nums">Rp {formatMoney(balance.totalLiabilities)}</strong></span><span>Total ekuitas <strong className="ml-2 font-mono tabular-nums">Rp {formatMoney(balance.totalEquity)}</strong></span><span>Laba berjalan <strong className="ml-2 font-mono tabular-nums">Rp {formatMoney(balance.currentEarnings)}</strong></span></div></div></>;
  }

  function trialView() {
    if (!trial) return null;
    return <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className={"flex flex-wrap items-center justify-between gap-3 border-b p-5 " + (trial.isBalanced ? "border-emerald-100 bg-emerald-50" : "border-red-100 bg-red-50")}><div><h3 className="font-semibold">Neraca Saldo</h3><p className="mt-1 text-xs text-muted">Saldo bersih seluruh akun pada periode yang dipilih.</p></div><span className={"font-mono text-sm font-bold tabular-nums " + (trial.isBalanced ? "text-emerald-700" : "text-red-700")}>Debit {formatMoney(trial.totalDebit)} · Kredit {formatMoney(trial.totalCredit)}</span></div><div className="overflow-x-auto"><table className="w-full min-w-[720px] text-sm"><thead className="bg-slate-50 text-left text-[11px] uppercase tracking-wide text-slate-400"><tr><th className="px-5 py-3">Kode / Akun</th><th className="px-3 py-3">Klasifikasi</th><th className="px-3 py-3 text-right">Saldo debit</th><th className="px-5 py-3 text-right">Saldo kredit</th></tr></thead><tbody>{trial.rows.map((row, index) => <tr className={index % 2 ? "bg-slate-50/60" : "bg-white"} key={row.accountId}><td className="px-5 py-3"><span className="font-mono font-semibold tabular-nums text-slate-600">{row.code}</span><span className="ml-2">{row.name}</span></td><td className="px-3 py-3 text-xs text-slate-500">{row.classification}</td><td className="px-3 py-3 text-right"><ReportAmount onClick={() => void openDrillDown(row.accountId)} value={row.debitBalance} /></td><td className="px-5 py-3 text-right"><ReportAmount onClick={() => void openDrillDown(row.accountId)} value={row.creditBalance} /></td></tr>)}</tbody><tfoot><tr className="border-t border-slate-200 bg-slate-50 font-bold"><td className="px-5 py-3" colSpan={2}>Total</td><td className="px-3 py-3 text-right"><ReportAmount value={trial.totalDebit} /></td><td className="px-5 py-3 text-right"><ReportAmount value={trial.totalCredit} /></td></tr></tfoot></table></div></section>;
  }
